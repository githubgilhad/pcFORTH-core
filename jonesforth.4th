...
: ':'	\ just colon
	...
	0 CHAR :		\ push the number 58 (ASCII code of colon) on the parameter stack
...
\ access to the word which we are currently compiling through the LAST pointer so we
\ can use that to compile a recursive call.
: RECURSE IMMEDIATE
	LAST D@	\ LAST points to the word being compiled at the moment
...
\		ALIGN		( round up to next multiple of 4 bytes for the remaining code )
	ELSE		( immediate mode )
		...
		WHILE
			...
			1D+		( increment address )
		REPEAT
		DROP		( drop the final " character )
		HERE D@ -D	( calculate the length )
		HERE D@		( push the start address )
		SWAP2 SWAP DROP	( addr len )
	THEN
;
...
(
	...
	and definitions will overwrite memory starting at the word.  We also need to set LAST to
...
: :NONAME
	...
	HERE D@		( current HERE value is the address of the codeword, ie. the xt )
...
: CSTRING	( addr len -- c-addr )
	...
	HERE D@ SWAP	( len saddr daddr len )
	CMOVE		( len )

	HERE D@ +	( daddr+len )
	0 SWAP C!	( store terminating NUL char )

	HERE D@ 		( push start address )
;
...
: UNUSED	( -- n )
	...
	HERE D@		( get current position in data segment )
	-
...
(
	...
	For example:
		...
		: DROP2 INLINE DROP INLINE DROP ;CODE

	will build an efficient assembler word DROP2 which contains the inline assembly code
